...
module.exports = {
    ...
    getReportsNoticiaAdmin: (request, response) => {
        const processData = async () => {
            let dependences = await dependenciaModel.getList();
            Promise.all(dependences);
            for (let i = 0; i < dependences.length; i++) {
                let dep = dependences[i];
                console.log(dep);
                if (dep?.id) {
                    //** Not admin  forms */

                    let notAdmin = await noticiaAdministrativaModel.getListIndicadoresForm(dep?.id);
                    Promise.all(notAdmin);
                    ...
                    let ws = XLSX.utils.aoa_to_sheet(pageAtivities);
                    XLSX.utils.book_append_sheet(wb, ws, "Formularios");

                    //** indicators  */

                    notAdmin = await noticiaAdministrativaModel.getListIndicadoresActAccInd(dep?.id);
                    let campos = await campoModel.getListCampos();
                    ...
                    for (let i = 0; i < notAdmin.length; i++) {
                        ...
                        indicatorsExist.push(arrTemp);

                    }
                    ws = XLSX.utils.aoa_to_sheet(indicatorsExist);
                    XLSX.utils.book_append_sheet(wb, ws, "ExistentesPoe");

                    let nameFile = dep?.nombre ? (dep?.nombre).replaceAll(' ', '_') : "other";
                    XLSX.writeFile(wb, `public/excelReports/noticiaAdministrativa/dependencias/${nameFile}.xlsx`, { numbers: XLSX_ZAHL_PAYLOAD, compression: true });


                }

            }
            return response.status(202).json({ "data": dependences });
        }
